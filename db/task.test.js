...
describe('db/task', function () {
  describe('#add()', function () {
    it('should add new task into user task list', function () {
      ...
      assert.equal(result.userId, userTask.userId);
      assert.equal(result.tasks[0].task, userTask.tasks[0].task);
      userTask = result;
    });
  });
  ...
  describe('#update()', function () {
    it('should update user task list', function () {
      ...
      assert.equal(result.userId, userTask.userId);
      assert.equal(result.tasks[0].task, 'x');
    });
  });
  describe('#refresh()', function () {
    it('should update refresh task list', function () {
      ...
      userTask = task.find(userTask.userId);
      ...
      assert.equal(result.tasks[1].important, true);
    });
  });
});
